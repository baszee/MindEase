...
class ChangePasswordActivity : AppCompatActivity() {
    ...
    private fun setupInitialUI() {
        if (isGoogleUser) {
            ...
            // FIX: Menggunakan ID tv_old_password_instruction
                ...
                is AuthResult.Success -> {
                    ...
                    if (isGoogleUser) {
                        if (!isReauthenticated) {
                            ...
                            binding.tvOldPasswordInstruction.text = getString(R.string.set_new_password_prompt)
                            Toast.makeText(this, "Verifikasi Sukses. Masukkan kata sandi baru Anda.", Toast.LENGTH_SHORT).show()
                            binding.btnSavePassword.text = getString(R.string.save)
                        } else {
                            Toast.makeText(this, getString(R.string.set_password_success), Toast.LENGTH_LONG).show()
                            Firebase.auth.signOut()
                            finish()
                        }
                    } else {
                        Toast.makeText(this, getString(R.string.password_updated_relogin), Toast.LENGTH_LONG).show()
                        Firebase.auth.signOut()
                        finish()
                    }
                }
    ...
    private fun setLoadingState(isLoading: Boolean) {
        ...
        binding.btnSavePassword.text = if (isLoading) getString(R.string.loading) else {
            if (isGoogleUser && !isReauthenticated) getString(R.string.verify_with_google) else getString(R.string.save)
        }
    }
}